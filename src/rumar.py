...
class Settings:
    ...
    db_path: str = None
      Used by: create, extract, reconcile
...
def iter_matching_files(top_rath: Rath, s: Settings, *, base_rath: Rath = None) -> Generator[Rath, None, None]:
    ...
    if base_rath is None:
        base_rath = top_rath
        s.update(  # NOTE: this loses the distinction between relative and absolute paths in the original input
...
def can_include_dir(path: Path, s: Settings, _relative_psx: str, base_path: Path) -> bool:
    if not s.included_files and not s.included_files_as_regex and not s.included_top_dirs:
        ...
        return True
    for abs_fglob_path in s.included_files:
        match abs_fglob_path.as_posix().split('/**', 1):
            case [prefix_fglob_psx, _]:
                # Recursive pattern /** found - use the segments before /** for matching;
                # any other pattern after /** will be handled by can_include_file()
                if (path.full_match(prefix_fglob_psx + '/**')  # match dir_path itself and any descendants
                        or path.full_match(prefix_fglob_psx)  # Python bug #139580 @ 3.13.7
                        or is_full_match_by_equivalent_segments(path, Path(prefix_fglob_psx))):  # match any ancestors
                    logger.log(DEBUG_14, f"|D ...{_relative_psx}  -- matches full glob '{abs_fglob_path.relative_to(base_path)}'")
                    return True
            case _:
                # No recursive pattern /** found - compare using the same number of initial segments
                if is_full_match_by_equivalent_segments(path, abs_fglob_path):
                    logger.log(DEBUG_14, f"|D ...{_relative_psx}  -- matches full glob '{abs_fglob_path.relative_to(base_path)}'")
                    return True
    if abs_top_dir := find_matching_abs_top_path(path, s.included_top_dirs):
        logger.log(DEBUG_13, f"=D ...{_relative_psx}  -- matches top dir '{abs_top_dir.relative_to(base_path)}'")
        return True
    if any(abs_top_dir.is_relative_to(path) for abs_top_dir in s.included_top_dirs):
...
def is_full_match_by_equivalent_segments(path: PurePath, abs_fglob_path: PurePath):
    """Full-matches using the same number of initial segments"""
    path_parts = path.parts
    abs_fglob_path_parts = abs_fglob_path.parts
    num_parts_to_compare = min(len(path_parts), len(abs_fglob_path_parts))
    equivalent_path = Path(*path_parts[:num_parts_to_compare])
    equivalent_glob_path = Path(*abs_fglob_path_parts[:num_parts_to_compare])
...
def can_include_file(path: Path, s: Settings, _relative_psx: str, base_path: Path) -> bool:
    if not s.included_files and not s.included_files_as_regex and not s.included_files_as_glob:
        ...
        return True
    if abs_fglob_path := find_matching_abs_fglob_path(path, s.included_files):
        logger.log(DEBUG_13, f"=F ...{_relative_psx}  -- matches full glob '{abs_fglob_path.relative_to(base_path)}'")
        return True
    if rx_str := find_matching_pattern(_relative_psx, s.included_files_as_regex):
        logger.log(DEBUG_14, f"=f ...{_relative_psx}  -- matches regex '{rx_str}'")
        return True
    if path != base_path:
        _relative_parent_psx = derive_relative_psx(path.parent, base_path, with_leading_slash=True)
        if rx_str := find_matching_pattern(_relative_parent_psx, s.included_dirs_as_regex):
            logger.log(DEBUG_14, f"=f ...{_relative_psx}  -- matches regex dir '{rx_str}'")
            return True
    if glob_str := find_matching_glob(path, s.included_files_as_glob):
        logger.log(DEBUG_13, f"=F ...{_relative_psx}  -- matches glob '{glob_str}'")
        return True
    if abs_top_dir := find_matching_abs_top_path(path, s.included_top_dirs):
...
def not_used(func):
    def wrapper(*args, **kwargs):
        raise RuntimeError(f"{func.__qualname__} is marked @not_used")
...
def find_matching_pattern(_relative_psx: str, patterns: Iterable[Pattern]):
    for rx in patterns:
...
def find_matching_glob(path: PurePath, globs: Iterable[str]):
    for glob_str in globs:
        if path.match(glob_str):
            return glob_str
    return None


def find_matching_abs_top_path(path: Path, abs_top_paths: Iterable[Path]):
    for abs_top_path in abs_top_paths:
        if path.is_relative_to(abs_top_path):
            return abs_top_path
    return None


def find_matching_abs_fglob_path(path: PurePath, abs_fglob_paths: Iterable[Path]):
    for abs_fglob_path in abs_fglob_paths:
        if path.full_match(abs_fglob_path):
            return abs_fglob_path
        # Python bug #139580 @ 3.13.7
        if abs_fglob_path.as_posix().endswith('/**') and path.full_match(abs_fglob_path.parent):
            return abs_fglob_path
    return None
...
class Rumar:
    ...
    def __init__(self, profile_to_settings: ProfileToSettings):
        ...
        self._brm: Broom = None  # initiated per profile in _init_for_profile to support db_path per profile
    ...
    def _set_rath_and_friends(self, rath: Rath | None, afresh=False):
        if rath is not None:
            ...
            lstat = rath.lstat_afresh() if afresh else rath.lstat()
    ...
    def _init_for_profile(self, profile: str, *, sweep=False):
        ...
        if sweep:
            self._brm = Broom(self._profile, self.s, self.lstat_cache, self._rdb)
    ...
    def _finalize_for_profile(self, *, identify_and_mark_deleted=True):
        if self.s.db_path:
            ...
            self._rdb.close_db()
            self._backup_to_bak_id_and_checksum = None
        self._brm and self._brm.close_db()
        self._rdb = None
        self._brm = None
        self._profile = None  # safeguard so that self.s will complain
        self._set_rath_and_friends(None)
        if self._warnings:
            ...
            try:
                ...
                if st.st_size == 0:
                    with suppress(OSError):
                        ...
                        logger.debug(f':- remove {checksum_file.__str__()!r}')
                else:
                    latest_checksum = checksum_file.read_bytes() if st.st_size == 64 else bytes.fromhex(checksum_file.read_text())
                    if self.s.db_path:
    ...
    def _create(self, op_reason: OpReason):
        ...
        if is_archive_created:
            ...
            if self.s.db_path:
                ...
                try:
                    checksum_file.write_bytes(self._rath_checksum)
                except OSError as e:
                    logger.info(f"{e}: skip writing checksum to {checksum_file.__str__()!r}")
    ...
    def _call_create_and_return_same_checksum_or_limit_reached(self, _create: Callable, attempt: int, attempt_limit: int, is_archive_created_lst: list[bool]) -> bool:
        ...
        if post_checksum == self._rath_checksum:
            is_archive_created_lst[0] = True
            return True
        else:
    ...
    def _reconcile_and_extract_for_run(self, run_id: int, top_archive_dir: Path | None, directory: Path | None, overwrite: bool, meta_diff: bool):
        ...
        for bak_dir, src_dir, src_path, bak_name in self._rdb.iter_paths_as_of_run(run_id, relative_top_dir):
            ...
            if directory:  # instead of source_dir
                target_path = directory / src_path
            else:
    ...
    def sweep_for_profile(self, profile, *, is_dry_run: bool):
        ...
        if ex := try_to_iterate_dir(s.backup_dir):
            ...
            return
        self._brm.scan_disk_and_mark_archive_files_for_deletion()
        self._brm.remove_archive_files_ided_by_sweep_and_mark_as_deleted(is_dry_run)
        self._finalize_for_profile(identify_and_mark_deleted=False)
...
class RumarDB:
    ...
    ddl = {
        ...
        'indexes': dedent('''\
            DROP INDEX IF EXISTS i_backup_reason;
        '''),
        ## IPython: import query_duration; %timeit query_duration.a() # => faster without the indexes
        # -- Core
        # -- For SELECT max(id) id FROM backup WHERE del_run_id = 0 GROUP BY src_id
        # CREATE INDEX IF NOT EXISTS i_backup_del_run_id_src_id_id ON backup (del_run_id, src_id, id);
        # -- For WHERE src_id = ? AND del_run_id = 0 ORDER BY b.id DESC
        # CREATE INDEX IF NOT EXISTS i_backup_src_id_del_run_id_id ON backup (src_id, del_run_id, id DESC);
        # -- For SELECT max(id) id FROM source_lc GROUP BY src_id
        # CREATE INDEX IF NOT EXISTS i_source_lc_src_id_id ON source_lc (src_id, id DESC);
        # -- Additional
        # -- For backup run_id joins
        # CREATE INDEX IF NOT EXISTS i_backup_run_id ON backup (run_id);
        # -- For run profile_id joins
        # CREATE INDEX IF NOT EXISTS i_run_profile_id_id ON run (profile_id, id);
        # -- For SELECT max(id) id FROM source_lc WHERE run_id <= 63 GROUP BY src_id
        # CREATE INDEX IF NOT EXISTS i_source_lc_run_id_src_id_id ON source_lc (run_id, src_id, id DESC);
        # -- For JOIN source_lc lc ON s.id = lc.src_id AND lc.reason != 'D'
        # CREATE INDEX IF NOT EXISTS i_source_lc_reason_src_id ON source_lc (reason, src_id, id);
        'view': {
            'v_backup': dedent('''\
    ...
    def init_run_datetime_iso_anew(self):
        ...
        while self.get_run_id(run_datetime_iso := self.make_run_datetime_iso()):
            sleep(0.25)
    ...
    def _load_data_into_memory(self):
        ...
        for bak_dir_id, src_id, bak_name, bak_id, blake2b_checksum in execute(self._cur, 'SELECT bak_dir_id, src_id, bak_name, id, blake2b FROM backup;'):
            self._backup_to_bak_id_and_checksum[(bak_dir_id, src_id, bak_name)] = (bak_id, blake2b_checksum)
    ...
    def _print_dicts(self):
        ...
        print(self._backup_to_bak_id_and_checksum)
    ...
    def profile_id(self):
        if self._profile_id is None:
            ...
            if not (profile_id := self._profile_to_id.get(profile)):
                profile_id = execute(self._cur, 'INSERT INTO profile (profile) VALUES (?) RETURNING id;', (profile,)).fetchone()[0]
    ...
    def src_dir_id(self):
        if self._src_dir_id is None:
            ...
            if not (src_dir_id := self._src_dir_to_id.get(src_dir)):
                src_dir_id = execute(self._cur, 'INSERT INTO source_dir (src_dir) VALUES (?) RETURNING id;', (src_dir,)).fetchone()[0]
    ...
    def bak_dir_id(self):
        if self._bak_dir_id is None:
            ...
            if not (bak_dir_id := self._bak_dir_to_id.get(bak_dir)):
                bak_dir_id = execute(self._cur, 'INSERT INTO backup_dir (bak_dir) VALUES (?) RETURNING id;', (bak_dir,)).fetchone()[0]
    ...
    def get_blake2b_checksum(self, bak_id: int) -> bytes | None:
        for row in execute(self._cur, 'SELECT blake2b FROM backup WHERE id = ?;', (bak_id,)):
            return row[0]
        return None
    ...
    def set_blake2b_checksum_for_path(self, archive_path: Path, blake2b_checksum: bytes):
        bak_dir = self.s.backup_dir.as_posix()
        bak_dir_id = self.bak_dir_id
        src_dir_id = self.src_dir_id
        src_path = derive_relative_psx(archive_path.parent, self.s.backup_dir)
        ...
        bak_id, old_blake2b_checksum = self._backup_to_bak_id_and_checksum[key]
        ...
        self._db.execute('UPDATE backup SET blake2b = ? WHERE id = ?;', (blake2b_checksum, bak_id))
        self._db.commit()
        self._backup_to_bak_id_and_checksum[key] = (bak_id, blake2b_checksum)
    ...
    def get_run_datetime_iso_to_profile(self, profile: str = None):
        and_profile = f"AND profile = '{profile}'" if profile else ''
        query = dedent(f"""\
        SELECT run_datetime_iso, profile
        FROM run
        JOIN profile ON profile_id = profile.id {and_profile}
        ORDER BY 1;""")
    ...
    def iter_paths_as_of_run(self, run_id: int, relative_top_dir: str = None):
        """Yields, as of run_id, latest available backups and their source files, if the source files aren't marked as deleted"""
        if relative_top_dir:
            where_src_path_like = '\nWHERE s.src_path LIKE ?'
            params = (run_id, run_id, f"{relative_top_dir}/%")
        else:
            where_src_path_like = ''
            params = (run_id, run_id,)
        query = dedent(f"""\
            SELECT bd.bak_dir, sd.src_dir, s.src_path, b.bak_name
            FROM "source" s
            JOIN source_dir sd ON s.src_dir_id = sd.id
            JOIN source_lc lc ON s.id = lc.src_id AND lc.reason != 'D' -- not deleted
            JOIN (SELECT max(id) id FROM source_lc WHERE run_id <= ? GROUP BY src_id) lx ON lc.id = lx.id -- recorded before or at the run
            JOIN backup b ON s.id = b.src_id
            JOIN (SELECT max(id) id FROM backup WHERE run_id <= ? AND del_run_id = 0) bx ON b.id = bx.id -- latest not deleted as of the run
            JOIN backup_dir bd ON b.bak_dir_id = bd.id {where_src_path_like};""")
    ...
    def is_bak_id_marked_as_active(self, bak_id: int):
        params = (bak_id,)
        for _ in execute(self._cur, 'SELECT 1 FROM backup WHERE id = ? AND del_run_id = 0;', params):
            return True
        return False
    ...
    def mark_backup_as_deleted(self, archive_path: Path, src_id: int = None):
        ...
        key = (self.bak_dir_id, src_id, archive_path.name)
        bak_id_and_checksum = self._backup_to_bak_id_and_checksum[key]
        if not bak_id_and_checksum:
            logger.warning(f"{key} not found in the database: {self.s.db_path}")
            return False
        params = (self.run_id, bak_id_and_checksum[0])
        cur = self._db.cursor()
        execute(cur, 'UPDATE backup SET del_run_id = ? WHERE id = ?;', params)
        cur.close()
        return True

    def mark_bak_id_as_deleted(self, bak_id: int):
        params = (self.run_id, bak_id)
        execute(self._cur, 'UPDATE backup SET del_run_id = ? WHERE id = ?;', params)
...
def execute(cur: sqlite3.Cursor | sqlite3.Connection, stmt: str, params: tuple | None = None, log=logger.debug):
    """:param cur: sqlite3.Cursor or sqlite3.Connection but not for INSERT, UPDATE, DELETE (AttributeError: 'sqlite3.Connection' object has no attribute 'rowcount')"""
...
class Broom:
    ...
    def is_archive(cls, name: str):
        return RX_ARCHIVE_SUFFIX.search(name)
    ...
    def _create_table_if_not_exists(self):
        ddl = dedent(f"""\
            CREATE {self.TEMPORARY} TABLE {self._table} (
                ...
                w TEXT NOT NULL,
                m TEXT NOT NULL,
                d_keep INTEGER NOT NULL DEFAULT 0,
                w_keep INTEGER NOT NULL DEFAULT 0,
                m_keep INTEGER NOT NULL DEFAULT 0,
                d_msg TEXT,
                w_msg TEXT,
                m_msg TEXT
            ) STRICT;""")
    ...
    def scan_disk_and_mark_archive_files_for_deletion(self):
        ...
        for rath in iterator:
            if self.is_archive(rath.name):
                ...
                logger.warning(f"{self._profile!r} {str(rath)!r}  is unexpected (not an archive)")
        for rath in sorted_files_by_stem_then_suffix_ignoring_case(old_enough_file_to_mdate):
            self.insert(rath, mdate=old_enough_file_to_mdate[rath])
        self.calc_cnt_and_update_keep_flag_for_each_period()
    ...
    def _calc_and_update_w_keep(self):
        ...
        stmt = dedent(f"""\
            ...
            chosen_from_day AS (
                ...
                FROM from_day
                WHERE rn <= {s.number_of_backups_per_week_to_keep}
            ),
            chosen_from_day_cnt AS (
                SELECT bak_parent, w, count(*) chosen_cnt
                FROM chosen_from_day
                GROUP BY bak_parent, w
            ),
            ...
            chosen AS (
                ...
                UNION ALL
                SELECT * FROM chosen_from_rest
            ),
            chosen_ranked AS (
                SELECT c.bak_parent, c.w, c.id, c.bak_cnt, row_number() OVER (PARTITION BY c.bak_parent, c.w ORDER BY c.id DESC) AS row_num
                FROM chosen c
            )
            SELECT bak_parent, w, id, bak_cnt, row_num
            FROM chosen_ranked
    ...
    def _calc_and_update_m_keep(self):
        ...
        stmt = dedent(f"""\
            ...
            chosen_from_dw AS (
                ...
                FROM from_dw
                WHERE rn <= {s.number_of_backups_per_month_to_keep}
            ),
            chosen_from_dw_cnt AS (
                SELECT bak_parent, m, count(*) chosen_cnt
                FROM chosen_from_dw
                GROUP BY bak_parent, m
            ),
            ...
            chosen_ranked AS (
                SELECT c.bak_parent, c.m, c.id, c.bak_cnt, row_number() OVER (PARTITION BY c.bak_parent, c.m ORDER BY c.id DESC) AS row_num
                FROM chosen c
            )
            SELECT bak_parent, m, id, bak_cnt, row_num
            FROM chosen_ranked
